    ...
    public function inscribir(Pago $pago)
{
    ...
    if ($pago->estado === 'PIG') {
        $grupos = GrupoExaman::where('estado', 'activo')->get();
...
protected function procesarInscripcionGrupo(Request $request, Pago $pago)
{
    ...
    $categoria = \App\Models\ExamenCategoriaAspira::where('nombre', $categoria_nombre)->first();
    if (!$categoria) {
...
protected function procesarInscripcionClases(Request $request, Pago $pago)
{
    $request->validate([
        'clases' => 'required|array',
        'clases.*' => 'exists:clase,id',
    ]);

    // Aquí deberías registrar las clases (por ejemplo, en una tabla intermedia)
    // Y marcar el pago como finalizado si es exitoso

    // Ejemplo (ajústalo a tu modelo real):
    foreach ($request->clases as $claseId) {
        // Asignar clase a estudiante si corresponde
    ...
    return redirect()->route('pagos.index')->with('success', 'Clases inscritas correctamente.');
}
