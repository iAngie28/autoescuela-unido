...
class GrupoExamanController extends Controller
{
    ...
    public function asignarEstudiante(Request $request): View
    {
        ...
        // Obtener los intentos previos de cada estudiante
        $inscripciones = ExamenSegip::latest('created_at')
            ->get()
            ->groupBy('id_est')
            ->map(fn($items) => $items->first());

        // Obtener pagos agrupados por estudiante y asociarles la categoría detectada
        $pagosPorEstudiante = [];

        $pagos = Pago::where('estado', 'PIG')->get();

        foreach ($pagos as $pago) {
            // Extraer categoría desde el detalle
            preg_match('/categoría (.*?) realizado/i', $pago->detalle, $matches);
            $categoriaNombre = $matches[1] ?? null;

            $categoriaId = null;
            if ($categoriaNombre) {
                $categoria = ExamenCategoriaAspira::where('nombre', trim($categoriaNombre))->first();
                if ($categoria) {
                    $categoriaId = $categoria->id;
                }
            }

            $pago->categoria_id = $categoriaId;

            $pagosPorEstudiante[$pago->id_est][] = $pago;
        }
    ...
    public function inscribir_grupo(Request $request)
{
    $request->validate([
        ...
        'id_pago' => 'nullable|exists:pago,id'
    ]);
    ...
    if ($existe) {
        // ← Solo redirecciona si fue por vista normal
        if (!$request->has('from_controller')) {
            return redirect()->back()->with('error', 'Este estudiante ya está inscrito en ese grupo.');
        }
        return ['success' => false, 'mensaje' => 'Ya está inscrito en ese grupo.'];
    }
    ...
    // Si fue llamado desde vista, redireccionamos
    if (!$request->has('from_controller')) {
        return redirect()->back()->with('success', 'Estudiante inscrito correctamente (Intento #' . $nuevoIntento . ')');
    }

    // Si fue llamado internamente, devolvemos resultado
    return ['success' => true, 'mensaje' => 'Estudiante inscrito correctamente (Intento #' . $nuevoIntento . ')'];
}
...
{
    $grupoExaman = GrupoExaman::findOrFail($id);

    $estudiantes = ExamenSegip::with(['estudiante.usuario', 'examenCategoriaAspira'])
        ->where('id_grupo', $id)
        ->get();

    return view('grupo-examan.show', compact('grupoExaman', 'estudiantes'));
}
