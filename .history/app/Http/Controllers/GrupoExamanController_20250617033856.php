...
public function exportarExcel($id)
{
    try {
        ...
        // 🧽 Copiar estilo y alto de la fila 9 original
        $filaReferencia = $filaInicio + $cantidad;
        $altoFila = $hojaFormulario->getRowDimension($filaReferencia)->getRowHeight();
        $estilos = [];

        for ($col = 1; $col <= 7; $col++) {
            $columna = Coordinate::stringFromColumnIndex($col);
            $estilos[$columna] = $hojaFormulario->getStyle("{$columna}{$filaReferencia}");
        }

        $fila = $filaInicio;
        foreach ($estudiantes as $i => $registro) {
            // Aplicar estilos y altura
            $hojaFormulario->getRowDimension($fila)->setRowHeight($altoFila);
            foreach ($estilos as $columna => $style) {
                $hojaFormulario->duplicateStyle($style, "{$columna}{$fila}");
            }
        ...
        // 🧩 Agrupar por categoría y aplicar en hojas CAT-*
        ...
        foreach ($categorias as $id_categoria => $grupoCat) {
            ...
            // Copiar estilos y altura de fila de la fila de referencia
            $filaReferenciaCat = $filaInicio + $cantidadCat;
            $altoFilaCat = $hojaCategoria->getRowDimension($filaReferenciaCat)->getRowHeight();
            $estilosCat = [];

            for ($col = 1; $col <= 7; $col++) {
                $columna = Coordinate::stringFromColumnIndex($col);
                $estilosCat[$columna] = $hojaCategoria->getStyle("{$columna}{$filaReferenciaCat}");
            }
            ...
            foreach ($grupoCat as $j => $registro) {
                ...
                $hojaCategoria->getRowDimension($filaCat)->setRowHeight($altoFilaCat);
                foreach ($estilosCat as $columna => $style) {
                    $hojaCategoria->duplicateStyle($style, "{$columna}{$filaCat}");
                }
