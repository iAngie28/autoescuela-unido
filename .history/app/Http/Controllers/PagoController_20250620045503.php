    ...
    public function inscribir(Pago $pago)
{
    ...
    $gruposActivos = GrupoExaman::where('estado', 'activo')->get();

    // Filtrar para excluir grupos en los que el estudiante ya está inscrito
    $gruposDisponibles = $gruposActivos->filter(function ($grupo) use ($pago) {
        return !DB::table('examen_segip')
            ->where('id_pago', '!=', $pago->id) // Evita conflicto con este mismo pago
            ->where('id_estudiante', $pago->id_estudiante)
            ->where('id_grupo', $grupo->id)
            ->exists();
    });

    return view('pago.inscribir', [
        'pago' => $pago,
