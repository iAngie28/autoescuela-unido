...
public function exportarExcel($id)
{
    ...
    // ✔️ Evita insertar filas por ahora, solo escribe directamente
    $fila = $filaInicio;
    ...
    // 📂 Agrupar por categoría y escribir en hojas existentes
    ...
    foreach ($categorias as $id_categoria => $grupoCat) {
        ...
        if (!$hojaCategoria) {
            continue; // Si no existe, no hacemos nada
        }

        // ✔️ También escribir directamente sin insertar filas
        $filaCat = $filaInicio;
    ...
    // ✅ Bloque try-catch para mostrar el error real si algo falla
    try {
        ...
        return response("Error al generar el Excel: " . $e->getMessage(), 500);
    }
}
