...
public function exportarExcel($id)
{
    try {
        ...
        // ✔️ Validación: eliminar combinaciones en la zona de inserción si las hay
        $mergedCells = $hojaFormulario->getMergeCells();
        foreach ($mergedCells as $range) {
            $hojaFormulario->unmergeCells($range);
        }

        // 🧩 Insertar filas antes de escribir estudiantes
        ...
        foreach ($categorias as $id_categoria => $grupoCat) {
            ...
            // ✔️ Descombinar si hay celdas combinadas en esa hoja
            foreach ($hojaCategoria->getMergeCells() as $range) {
                $hojaCategoria->unmergeCells($range);
            }

            // 🧩 Insertar filas antes de escribir
