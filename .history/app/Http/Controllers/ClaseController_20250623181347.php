...
public function asingar_estudiante_clase(Request $request): View
{
    ...
    // Preparar pagos por estudiante (solo estado PIP)
    $pagosPorEstudiante = [];

    foreach ($usuariosEstudiante as $usuario) {
        if ($usuario->estudiante) {
            $idEstudiante = $usuario->estudiante->id;

            $pagos = Pago::where('id_est', $idEstudiante)
                ->where('estado', 'PIP')
                ->get(['id', 'detalle']);

            $pagosPorEstudiante[$idEstudiante] = $pagos->toArray();
        }
    }

    return view('clase.asignar_clase', compact('clases', 'usuariosEstudiante', 'pagosPorEstudiante'));
}
    ...
    public function asignar_clase(Request $request, $id)
{
    try {
        ...
        if ($request->filled('id_pago')) {
            ...
            $totalEsperado = isset($matches[1]) ? (int)$matches[1] : 0;

            // Contar clases asignadas con ese id_pago
            $clasesAsignadas = Clase::where('id_pago', $pago->id)->count();

            // Si se alcanzó el total esperado, marcar como finalizado
            if ($clasesAsignadas >= $totalEsperado && $totalEsperado > 0) {
