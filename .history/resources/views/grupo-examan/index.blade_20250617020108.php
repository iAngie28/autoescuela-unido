                                        ...
                                        <td class="py-3 px-6 text-center">
                                            <div class="flex items-center justify-center space-x-2">
                                                ...
                                                <form action="{{ route('grupo-examen.destroy', $grupoExaman->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
                                                    <button type="submit" onclick="event.preventDefault(); confirm('Seguro que desea eliminar?') ? this.closest('form').submit() : false;" class="text-red-500 hover:scale-110">🗑
                                                        Eliminar</button>
                                                </form>
                                                <form action="{{ route('grupo-examen.exportar-excel', $grupoExaman->id) }}" method="POST" target="_blank">
    @csrf
    <button type="submit" class="text-green-600 hover:scale-110">📥 Generar Excel</button>
</form>
