                            ...
                            <tbody class="text-gray-600 text-sm">
                                @foreach ($estudiantes as $estudiante)
                                    ...
                                    <tr class="border-b border-gray-200 hover:bg-gray-100">
                                        ...
                                        <td class="py-3 px-6 text-center">
                                            <select name="id_pago"
                                                class="select-pago w-full px-2 py-1 border border-gray-300 rounded-md"
                                                required>
                                                <option value="">Seleccionar</option>
                                                @foreach ($pagosPorEstudiante[$estudiante->id] ?? [] as $pago)
                                                    @php
                                                        preg_match(
                                                            '/categoría (.*?) realizado/i',
                                                            $pago->detalle,
                                                            $match,
                                                        );
                                                        $cat = $match[1] ?? '';
...
document.addEventListener('DOMContentLoaded', () => {
    document.querySelectorAll('.select-pago').forEach(select => {
        select.addEventListener('change', function () {
            const selectedOption = this.options[this.selectedIndex];
            const categoriaNombre = selectedOption.getAttribute('data-categoria');
            if (!categoriaNombre) return;

            const row = this.closest('tr');
            const selectCategoria = row.querySelector('select[name="categoria_id"]');
            if (!selectCategoria) return;

            const options = selectCategoria.options;
            for (let i = 0; i < options.length; i++) {
                const optText = options[i].textContent.trim().toLowerCase();
                if (optText === categoriaNombre.toLowerCase()) {
