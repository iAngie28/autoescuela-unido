<x-guest-layout>
    <form method="POST" action="{{ route('usuarios.update', $user->id) }}">
        ...
        <div class="mt-4">
            ...
            <select id="id_rol" name="id_rol" class="block mt-1 w-full rounded" required>
                ...
                @foreach($roles as $rol)
                    <option value="{{ $rol->id }}" {{ old('id_rol', $user->id_rol) == $rol->id ? 'selected' : '' }}>{{ $rol->nombre }}</option>
                @endforeach
            </select>
        ...
        <div class="flex items-center justify-end mt-6">
            <a href="{{ route('usuarios.index') }}" class="underline text-sm text-gray-600 hover:text-gray-900 rounded-md mr-4">
                Cancelar
            </a>
            <x-primary-button>
                {{ __('Guardar cambios') }}
