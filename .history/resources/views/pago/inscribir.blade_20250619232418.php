...
document.addEventListener('DOMContentLoaded', function () {
    ...
    checkboxes.forEach((checkbox, i) => {
        checkbox.addEventListener('change', function () {
            ...
            // Quita estilos de todos
            labels.forEach(label => {
                label.classList.remove('bg-blue-900', 'text-white', 'border-blue-700');
                label.classList.add('bg-white', 'text-black', 'border-gray-300');
            });

            // Aplica estilos solo a los seleccionados
            checkboxes.forEach((cb, idx) => {
                if (cb.checked) {
                    labels[idx].classList.remove('bg-white', 'text-black', 'border-gray-300');
                    labels[idx].classList.add('bg-blue-900', 'text-white', 'border-blue-700');
                }
            });
        });
    });
