...
@section('content')
    ...
    @if ($pago->estado === 'finalizado')
        ...
        <div class="flex flex-col items-center">
            <span>↩️</span>
            <form action="{{ route('pagos.anular', $pago->id) }}" method="POST"
                  onsubmit="return confirm('¿Seguro que deseas anular este pago?');">
                @csrf
                @method('PUT')
                <button type="submit" class="text-red-500 text-sm">Anular</button>
            </form>
        </div>
    @else
        ...
        <div class="flex flex-col items-center">
            <span>✏</span>
            <a href="{{ route('pagos.inscribir', $pago->id) }}" class="text-green-500 text-sm">Inscribir</a>
        </div>
