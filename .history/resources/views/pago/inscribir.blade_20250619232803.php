                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        @foreach ($clases as $clase)
                            <label for="clase_{{ $clase->id }}"
                                class="clase-label block border border-gray-300 rounded px-4 py-3 cursor-pointer transition">
                                <input type="checkbox" name="clases[]" value="{{ $clase->id }}"
                                    id="clase_{{ $clase->id }}" class="clase-checkbox hidden">
                ...
                checkboxes.forEach((checkbox) => {
                    checkbox.addEventListener('change', () => {
                        ...
                        checkboxes.forEach(cb => {
                            ...
                            label.classList.remove('bg-[#1E293B]', 'text-white',
                                'border-[#1E293B]');
                            label.classList.add('bg-white', 'text-black', 'border-gray-300');
                        });

                        // Apply styles to checked
                        checkboxes.forEach(cb => {
                            if (cb.checked) {
                                const label = cb.closest('.clase-label');
                                label.classList.remove('bg-white', 'text-black',
                                    'border-gray-300');
                                label.classList.add('bg-[#1E293B]', 'text-white',
                                    'border-[#1E293B]');
                            }
                        });
                    });
