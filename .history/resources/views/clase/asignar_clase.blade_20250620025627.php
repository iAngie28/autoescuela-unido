...
@section('content')
                                ...
                                @foreach ($clases as $clase)
                                    <tr data-estado="{{ strtolower($clase->estado) }}"
                                        ...
                                        <form action="{{ route('clases.asignar_clase', $clase->id) }}" method="POST"
                                            ...
                                            <td class="py-3 px-6 text-center">
                                                <select name="id_pago" class="form-select id_pago_select"
                                                    data-row="{{ $clase->id }}">
    ...
    document.querySelectorAll('select[name="nid_est"]').forEach(estSelect => {
        estSelect.addEventListener('change', function () {
            const estudianteId = this.value;
            const form = this.closest('form');
            const claseId = form.getAttribute('action').split('/').pop();
            const pagoSelect = form.querySelector('select[name="id_pago"]');
            ...
            if (pagosPorEstudiante[estudianteId]) {
                pagosPorEstudiante[estudianteId].forEach(pago => {
                    ...
                    option.textContent = `#${pago.id} - ${pago.detalle}`;
